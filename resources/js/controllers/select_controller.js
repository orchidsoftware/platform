...
export default class extends ApplicationController {
    ...
    connect() {
        const select = this.hasSelectTarget
            ? this.selectTarget
        ...
        const options = {
            ...
            placeholder:
                select.getAttribute("placeholder") === "false"
                    ? ""
            ...
            maxItems:
                select.getAttribute("maximumSelectionLength") ||
                select.getAttribute("data-maximum-selection-length") ||
                (select.hasAttribute("multiple") ? null : 1),
            render: {
                option_create: (data, escape) =>
                    `<div class="create">${this.messageAddValue} <strong>${escape(data.input)}</strong>&hellip;</div>`,
                no_results: () =>
                    `<div class="no-results">${this.messageNotfoundValue}</div>`,
            },
