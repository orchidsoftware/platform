<div data-controller="attach"
     ...
     data-attach-upload-url-value="{{ $uploadUrl ?? route('orchid.files.upload') }}"
     data-attach-sort-url-value="{{ $sortUrl ?? route('orchid.files.sort') }}"
...
>
    ...
    <template data-attach-target="template">
        <div class="pip col position-relative">
            ...
            <button class="btn-close rounded-circle bg-body border shadow position-absolute end-0 top-0"
