<div class="wrapper-md">
    <div class="bg-white">
        <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
            <label class="col-lg-2 control-label">{{trans('platform::systems/roles.name')}}</label>
            <div class="col-lg-10">
                <input type="text" name="name" class="form-control"
                       value="{{ old('name', ($role ? $role->name : '')) }}">
                <small class="form-text text-muted m-b-none">{{trans('platform::systems/roles.name_help')}}
                </small>
            </div>
        </div>
        <div class="line line-dashed b-b line-lg"></div>
        <div class="form-group{{ $errors->has('slug') ? ' has-error' : '' }}">
            <label class="col-lg-2 control-label">{{trans('platform::systems/roles.slug')}}</label>
            <div class="col-lg-10">
                <input type="text" class="form-control" name="slug"
                       value="{{ old('slug', ($role ? $role->slug : '')) }}">
                <small class="form-text text-muted m-b-none">{{trans('platform::systems/roles.slug_help')}}</small>
            </div>
        </div>
