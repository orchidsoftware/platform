...
@section('content')
        ...
        <div class="row">
            ...
            <div class="col-sm-6 col-xs-12 text-right">
                <div class="btn-group" role="group" aria-label="...">
                    ...
                    <button type="button" class="btn btn-link"  @if($method == 'create') disabled @endif><i class="ion-ios-trash-outline  fa fa-2x"></i></button>
                </div>
            </div>
    ...
    <section class="bg-white-only b-l bg-auto no-border-xs">
        ...
        <form class="form-horizontal" id="form-group"
              ...
              @if($method == 'create')
              action="{{route($route->get('store',''))}}"
              @elseif($method == 'update')
              action="{{route($route->get('update',''),$model->$slug)}}"
              @endif
        ...
        @if($method != 'create')
            <form id="form-group-remove" action="{{route($route->get('destroy',''),$model->$slug)}}" method="POST" style="display: none;">
                {{ csrf_field() }}
