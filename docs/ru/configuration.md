...
Всё параметры располагаются в директории `config`, а основным файлом для платформы является 
...
Платформа прекрасно понимает, что не может покрыть всех задач разработчика, по этому предоставляет возможность
...
не хватает, вместо того, что бы изменять редактор в каждом используемом файле, вам достаточно будет изменить 
