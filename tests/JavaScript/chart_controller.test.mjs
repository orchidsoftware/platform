...
const source = await readFile(new URL('../../resources/js/controllers/chart_controller.js', import.meta.url), 'utf8');
...
async function harness(readyState = 'complete') {
    ...
    const definitions = ['BarChart', 'LineChart', 'MixedChart', 'PercentageChart', 'PieChart'];
    const exports = definitions.map(type => ({
        make(canvas) {
            const record = { type, canvas, calls: [], destroyed: false, downloads: 0 };
            builds.push(record);
            const builder = {};
            for (const method of ['labels', 'height', 'colors', 'gradient', 'smooth', 'legend', 'stacked', 'maxSlices', 'dataset', 'marker', 'region']) {
                builder[method] = value => { record.calls.push([method, value]); return builder; };
            }
            builder.render = () => ({
    ...
    await module.link(specifier => {
        if (specifier === './application_controller') {
            return new vm.SyntheticModule(['default'], function () { this.setExport('default', class {}); }, { context });
        }
        assert.equal(specifier, '@orchidsoftware/charts');
        return new vm.SyntheticModule(definitions, function () {
            definitions.forEach((name, index) => this.setExport(name, exports[index]));
        }, { context });
    });
    ...
    controller.configValue = {
        type: 'line',
        options: { height: 300, gradient: { fromOpacity: 0.25, toOpacity: 0.02 }, legend: false },
        data: { labels: ['Mon', 'Tue'], datasets: [{ name: 'Visits', values: [12, 18] }] },
    };
...
test('passes the PHP options and native data through for all five chart types', async () => {
    for (const type of ['line', 'bar', 'mixed', 'pie', 'percentage']) {
        ...
        const datasets = type === 'mixed'
            ? [{ name: 'Actual', chartType: 'bar', values: [12, 18] }, { name: 'Plan', chartType: 'line', values: [15, 16], smooth: true, gradient: false }]
        ...
        assert.deepEqual(builds[0].calls, [
            ['labels', ['Mon', 'Tue']], ['height', 300], ['legend', false],
...
test('forwards gradient objects, markers and regions without translation', async () => {
    const { controller, builds } = await harness();
    controller.configValue.data.markers = [{ label: 'Target', value: 20, lineStyle: 'dashed' }];
    controller.configValue.data.regions = [{ label: 'Range', range: [5, 15] }];
    controller.connect();
    assert.deepEqual(builds[0].calls.find(([method]) => method === 'gradient')[1], { fromOpacity: 0.25, toOpacity: 0.02 });
...
test('waits for stylesheets and cancels pending rendering on disconnect', async () => {
    const { controller, document, listeners, builds } = await harness('loading');
    controller.connect();
...
test('replaces the chart when options or the canvas target change', async () => {
    ...
    controller.configValueChanged();
    assert.equal(builds[0].destroyed, true);
    assert.deepEqual(builds[1].calls.find(([method]) => method === 'height'), ['height', 420]);
...
test('exports the current instance and prevents link navigation', async () => {
    ...
    controller.connect();
    let prevented = false;
    controller.export({ preventDefault() { prevented = true; } });
